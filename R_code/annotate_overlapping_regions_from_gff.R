...
pick_first_pref <- function(values, src, prefer=c("gene","mRNA")){ord<-order(match(src,prefer),na.last=TRUE);v<-values[ord];v<-v[!is.na(v)&nzchar(v)];if(length(v)) v[[1]] else NA_character_}

read_regions <- function(f){
  stopifnot(file.exists(f))
  x <- read_tsv(f, show_col_types = FALSE)
  CH <- pick_col(x,"chrom","seqid","CHROM"); ST <- pick_col(x,"start","bin_start","START","begin"); ED <- pick_col(x,"end","bin_end","END","stop"); LB <- pick_col(x,"label","LABEL")
  x %>% transmute(CHROM=canon_chr(.data[[CH]]),START=as.integer(.data[[ST]]),END=as.integer(.data[[ED]]),LABEL=canon_lab(.data[[LB]])) %>% filter(LABEL %in% c("sweep","balancing"), START<=END)
}
...
parse_gff_genes <- function(gff){
  ...
  rows <- list(); i <- 0L
  add <- function(gene,chrom,s,e,src,A){if(is.na(gene)||gene=="") return(); i<<-i+1L; rows[[i]]<<-list(GENE=gene,CHROM=canon_chr(chrom),START=as.integer(s),END=as.integer(e),SRC=src,NAME=A[["Name"]] %||% A[["gene_name"]] %||% A[["gene"]],PRODUCT=A[["product"]] %||% A[["Product"]],NOTE=A[["note"]] %||% A[["Note"]],DBXREF=A[["Dbxref"]] %||% A[["db_xref"]],BIOTYPE=A[["gene_biotype"]] %||% A[["biotype"]])}
  repeat{
  ...
  df %>% group_by(GENE,CHROM) %>% summarise(START=min(START,na.rm=TRUE),END=max(END,na.rm=TRUE),NAME=pick_first_pref(NAME,SRC),PRODUCT=pick_first_pref(PRODUCT,SRC),NOTE=pick_first_pref(NOTE,SRC),DBXREF=pick_first_pref(DBXREF,SRC),BIOTYPE=pick_first_pref(BIOTYPE,SRC),.groups="drop") %>% filter(!is.na(START),!is.na(END),START<=END)
}

regions_to_genes <- function(reg,genes,nearest_kb=0){
  if(nrow(reg)==0||nrow(genes)==0) return(tibble())
  gr_reg <- GRanges(reg$CHROM,IRanges(reg$START,reg$END)); gr_gen <- GRanges(genes$CHROM,IRanges(genes$START,genes$END))
  ov <- findOverlaps(gr_reg,gr_gen)
  res <- tibble(CHROM=reg$CHROM[queryHits(ov)],R_START=reg$START[queryHits(ov)],R_END=reg$END[queryHits(ov)],LABEL=reg$LABEL[queryHits(ov)],GENE=genes$GENE[subjectHits(ov)])
  if(nearest_kb>0){
    has <- unique(queryHits(ov)); miss <- setdiff(seq_along(gr_reg),has)
    if(length(miss)>0){
      nn <- suppressWarnings(nearest(gr_reg[miss],gr_gen,select="arbitrary")); ok <- !is.na(nn)
      if(any(ok)){
        d <- distance(gr_reg[miss][ok],gr_gen[nn[ok]])
        within <- which(d <= nearest_kb*1000)
        if(length(within)>0){
          add <- tibble(CHROM=as.character(seqnames(gr_reg[miss][ok][within])),R_START=start(gr_reg[miss][ok][within]),R_END=end(gr_reg[miss][ok][within]),LABEL=reg$LABEL[miss][ok][within],GENE=genes$GENE[nn[ok][within]])
...
annotate_categories <- function(df){
  ...
  cats <- lapply(txt,function(s){hit <- names(cat_patterns)[vapply(cat_patterns,function(p) grepl(p,s,perl=TRUE),logical(1))]; if(length(hit)==0) NA_character_ else paste(unique(hit),collapse=";")})
...
make_barplot <- function(tab,title,filebase){
  ...
  p <- ggplot(tab,aes(x=reorder(Category,Count),y=Count,fill=LABEL))+geom_col(position="dodge")+coord_flip()+labs(title=title,x=NULL,y="Gene count")+theme_minimal(base_size=12)
  ggsave(file.path(outdir,paste0(filebase,".pdf")),p,width=7,height=5); ggsave(file.path(outdir,paste0(filebase,".png")),p,width=7,height=5,dpi=300)
}
count_cats <- function(df) df %>% filter(!is.na(PRIMARY_CAT)) %>% count(LABEL,Category=PRIMARY_CAT,name="Count") %>% arrange(desc(Count))
rep_pick <- function(df,per_cat=3) df %>% filter(!is.na(PRIMARY_CAT)) %>% group_by(LABEL,PRIMARY_CAT) %>% slice_head(n=per_cat) %>% ungroup() %>% select(LABEL,PRIMARY_CAT,GENE,NAME,PRODUCT,REGION)

reg1 <- read_regions(tier1_file)
...
t1 <- map1 %>% left_join(annot, by="GENE") %>% mutate(REGION=paste0(CHROM,":",R_START,"-",R_END))
t1c <- annotate_categories(t1)
write_tsv(t1c, file.path(outdir, "Tier1_region_gene_annotations.tsv"))
t1_counts <- count_cats(t1c)
...
summary_txt <- tibble(item=c("Tier1_regions","Tier1_genes"), value=c(n_distinct(t1c$REGION), n_distinct(t1c$GENE)))
write_tsv(summary_txt, file.path(outdir, "summary_counts_tier1.tsv"))
cat("Done.\nOutputs in: ", outdir, "\n")
